...
async def ask_to_select(channelOrInteract: t.Union[discord.TextChannel, discord.DMChannel, discord.GroupChannel, Interaction], choosers: t.List[discord.User], options: t.List[t.Any], *, question="Which one do you select?", ephemeral=True, show_chosen=False, delete_after=False):
    ...
    while not selected:
        ...
        if not timed_out:
            selectedIndex = int(view.chosenIndex)
...
async def send_embed(channel: typing.Union[discord.Interaction, discord.TextChannel, discord.DMChannel, discord.GroupChannel], content=None, title="", description="", fieldNameToDescriptionDicti={}, fieldsInline=True, color=Color.dark_blue(), view=None):
    """An async function to send an embed to a channel."""
    embed = discord.Embed(title=title, description=description, colour=color)
    for key, value in fieldNameToDescriptionDicti.items():
        embed.add_field(name=key, value=value, inline=fieldsInline)

    if (isinstance(channel, discord.Interaction)):
        return await channel.response.send_message(content=content, embed=embed, view=view)
